...
void pipe_read_cb(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf) 
{
    if (nread > 0) 
    {
...
void on_pipe_connection(uv_stream_t *server, int status) 
{
    ...
    if(tunnel_mgr == NULL)
    {
        log_msg(
            LOG_ERR, 
            "Received client pipe connection, but pipe server missing tunnel manager data"
        );
    ...
    if (uv_accept(server, (uv_stream_t*) client) == 0) 
    {
        ...
        uv_read_start((uv_stream_t*) client, tunnel_manager_alloc_buffer, pipe_read_cb);
    }
    else 
    {
        uv_close((uv_handle_t*) client, pipe_close_cb);
...
{
    if (send(tunnel_mgr->tm_sock_fd, TM_STOP_MSG, strlen(TM_STOP_MSG), 0) == -1) 
    {
